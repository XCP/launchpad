...
import { SOCIAL_ICONS } from "@/app/[lang]/[asset]/_components/launch-metadata";
...
export function ShareButton({
  ...
  const text = t("{asset}: {headline} on xcp.fun", { asset, headline });
  ...
  return (
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen} title={t("Share this launch")}>
        <div className="px-2 pb-2">
          <p className="mb-3 text-xs text-gray-500 dark:text-gray-400">
            {t("Copy the link, or post it straight to X.")}
          </p>
...
export function AnnouncedAgo({
  ...
  return (
    ...
    >
      {unconfirmed ? t("unconfirmed") : timeAgo(at!, t)}
    </a>
  );
}
  ...
  return (
    <>
      <button
        type="button"
        onClick={() => setOpen(true)}
        aria-label={t("View {asset} artwork full size", { asset })}
