...
export function PriceChart({
  ...
  if (!hasAny) {
    return (
      <p className="rounded-md bg-gray-50 dark:bg-gray-800/60 p-4 text-center text-sm text-gray-500 dark:text-gray-400">
        {t("No trades yet — nothing has changed hands since the pool opened.")}
      </p>
    );
  }
  ...
  const bucketWord = bucketLabel === "hour" ? t("hour") : t("day");
    ...
    <div className="relative">
        ...
        <div className="flex items-center gap-1">
          ...
          {xcpUsd !== null && (
            <button
              ...
              title={
                usdHistoryReady ? undefined : t("Loading historical XCP/USD")
              }
      ...
      {points.length === 0 ? (
        ...
        <svg
          ...
          role="img"
          aria-label={t("{asset} price against {unit}, {n} {bucket} {kind}", {
            asset,
            unit: inUsd ? code : "XCP",
            n: points.length,
            bucket: bucketWord,
            kind: mode === "line" ? t("points") : t("candles"),
          })}
        >
          {yTicks.map((t) => (
            <g key={t.v}>
