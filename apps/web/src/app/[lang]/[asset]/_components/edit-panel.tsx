...
export function EditPanel({ asset }: { asset: string }) {
  ...
  if (proofStatus === "failed") {
    return (
      <div className="mt-4 rounded-3xl border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/40 p-4 text-sm text-red-800 dark:text-red-300">
        <p className="font-semibold">{t("Editing is locked for this session")}</p>
        <p className="mt-1 text-xs">
          {t(
            "Your wallet supplied a connection signature that didn't verify, so we can't confirm this address is really yours. Disconnect and reconnect to try again — nothing else on the site is affected.",
          )}
        </p>
      </div>
    );
  ...
  const save = async () => {
    ...
    try {
      ...
      setState({
        status: "error",
        error: e instanceof Error ? e.message : t("Something went wrong"),
      });
    }
  };
  ...
  return (
    <details className="mt-4 rounded-3xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900" onToggle={prefill}>
      <summary className="cursor-pointer p-4 text-sm font-semibold text-gray-700 dark:text-gray-300">
        {t("Edit token info")}{" "}
        <span className="font-normal text-gray-400 dark:text-gray-500">{t("(you own this asset)")}</span>
      </summary>
      <div className="space-y-4 border-t border-gray-100 dark:border-gray-800 p-4">
        <p className="text-xs text-gray-500 dark:text-gray-400">
          {t(
            "What xcp.fun shows for this launch, and what the hosted metadata says if your on-chain description points here. The on-chain field itself is locked forever; everything behind it is yours to curate.",
          )}
        </p>
        <div>
        ...
        <div>
          <label htmlFor="edit-image" className="text-sm font-medium text-gray-700 dark:text-gray-300">
            {t("Replace image")}{" "}
            <span className="font-normal text-gray-400 dark:text-gray-500">{t("(optional)")}</span>
          </label>
          <input
            id="edit-image"
            type="file"
          ...
          {animatedWebp && (
            <p className="mt-2 text-xs text-amber-700 dark:text-amber-400">
              {t(
                "Animated WEBP shows as a still frame in the announce channel — Telegram cannot play one. A GIF moves there too.",
              )}
            </p>
          )}
        </div>
        ...
        {state.status === "saved" && (
          <p className="rounded-md border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-950/40 p-2 text-sm text-green-700 dark:text-green-400">
            {t("Saved. Changes are live; other open pages may take a minute to refresh.")}
          </p>
        )}
        <button
          type="button"
