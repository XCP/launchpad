...
export function ActivityTabs({
  ...
  const setParams = (next: typeof tab, p: number) => {
    setTab(next);
    setPage(p);
  };
  ...
  const tabLabel = (key: typeof tab) =>
    key === "minters"
      ? `${t("Holders")} (${minters.length})`
      : key === "mempool"
        ? `${t("Mempool")}${minting ? (roomState ? ` (${pending.length})` : "") : pendingOrders.length > 0 ? ` (${pendingOrders.length})` : ""}`
        : key === "trades"
          ? `${t("Trades")}${tradePage ? ` (${tradePage.total})` : ""}`
          : key === "holders"
            ? `${t("Holders")}${liveHolderCount !== null ? ` (${liveHolderCount})` : ""}`
    ...
    <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900">
        ...
        <Tabs value={tab} onValueChange={(v) => setParams(v as typeof tab, 1)}>
          <div className="flex items-center justify-between gap-2 border-b border-gray-200 dark:border-gray-800 p-2">
            <SegmentedList variant="card">
              {tabs.map((key) => (
                <SegmentedTrigger key={key} value={key} variant="card" grow={false}>
                  {tabLabel(key)}
                </SegmentedTrigger>
              ))}
            </SegmentedList>
                        ...
                        <span className="relative z-10 flex items-center gap-2">
                          ...
                          {freshness?.noHistory.has(r.source) && (
                            <span
                              ...
                              title={t("The explorer has never seen this address do anything on chain.")}
                            >
                              {t("no history")}
                            </span>
                          )}
      ...
      {tab === "mempool" && minting &&
        (!roomState ? (
          ...
          <p className="p-6 text-center text-sm text-gray-500 dark:text-gray-400">
            {t("Nothing queued — every mint so far has confirmed.")}
          </p>
        ) : (
          <>
            <p className="border-b border-gray-100 dark:border-gray-800 bg-amber-50/50 dark:bg-amber-950/40 px-4 py-2 text-xs text-amber-800 dark:text-amber-300">
              {t("Unconfirmed and provisional — mempool mints can exceed what's left, get replaced, or never confirm. Only what lands on-chain counts.")}
            </p>
            <ul className="divide-y divide-gray-100 dark:divide-gray-800">
      ...
      {tab === "mempool" && !minting &&
        (pendingOrders.length === 0 ? (
          <p className="p-6 text-center text-sm text-gray-500 dark:text-gray-400">
            {t("No orders for {asset} are waiting in the mempool.", { asset })}
          </p>
        ) : (
          <>
            <p className="border-b border-gray-100 dark:border-gray-800 bg-amber-50/50 dark:bg-amber-950/40 px-4 py-2 text-xs text-amber-800 dark:text-amber-300">
              {t("Unconfirmed and provisional — these orders join the book only after confirmation.")}
            </p>
            <ul className="divide-y divide-gray-100 dark:divide-gray-800">
                ...
                return (
                  <li key={o.txHash} className="flex items-center justify-between gap-3 px-4 py-2 text-sm">
                    ...
                    <a href={`https://xcp.io/tx/${o.txHash}`} target="_blank" rel="noreferrer" className={`shrink-0 font-medium hover:underline ${buy ? "text-green-700 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}>
                      {buy ? t("Buy pending") : t("Sell pending")}
                    </a>
                  </li>
                );
              })}
              ...
              <table className={PAIR_TABLE}>
                <thead>
                  <tr className="border-b border-gray-100 dark:border-gray-800 text-left text-[10px] font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">
                    <th className="px-4 py-2">{t("Side")}</th>
                    ...
                    <th className="px-4 py-2 text-right">{t("Venue / time")}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                  {trades.map((trade) => {
                    const tokens = tokenQty(trade.tokenRaw, divisible);
                    const walletRole = tradeRoleForAddress(
                      {
                        address: trade.addr,
                        counterpartyAddress: trade.counterpartyAddr,
                        venue: trade.via,
                      },
                      address,
                    );
                    ...
                    // placeholder string.
                    const hasTime = trade.time > 0;
                    const at = hasTime ? new Date(trade.time * 1000) : null;
                    return (
                      <tr
                        key={trade.key}
                        className={
                        ...
                        title={
                          walletRole === "counterparty"
                            ? t("Your connected wallet's resting order participated in this fill")
                            : walletRole
                              ? t("Trade from your connected wallet")
                              : undefined
                        }
                      >
                        <td className={`whitespace-nowrap px-4 py-2 font-medium ${trade.buy ? "text-green-700 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}>
                          {walletRole && (
                            <span className="sr-only">
                              {t("Your connected wallet participated in this trade.")} {" "}
                            </span>
                          )}
                        ...
                        <td className="px-3 py-2">
                          <span className="flex items-center gap-1.5 whitespace-nowrap">
                            <LaunchpadAddressHoverCard
                              source={trade.addr}
                              asset={asset}
                              balanceRaw={holderBalance.get(trade.addr)}
                            ...
                            >
                              <Identicon address={trade.addr} />
                              {shortAddress(trade.addr)}
                            </LaunchpadAddressHoverCard>
                            <DevBadge address={trade.addr} issuerSource={issuerSource} />
                          </span>
                        </td>
                        <td className="whitespace-nowrap px-4 py-2 text-right text-xs text-gray-500 dark:text-gray-400">
                          <a
                            href={`https://xcp.io/tx/${trade.txHash}`}
                            target="_blank"
                            rel="noreferrer"
                            title={
                              at
      ...
      {tab === "holders" &&
        (!holders ? (
          <p className="p-6 text-center text-sm text-gray-400 dark:text-gray-500">{t("Loading holders…")}</p>
        ) : holderRows.length === 0 ? (
          <p className="p-6 text-center text-sm text-gray-500 dark:text-gray-400">{t("No holders found.")}</p>
        ) : (
          <>
                ...
                return (
                  ...
                  >
                        ...
                        <>
                          ...
                          {lpAsset ? (
                            <a
                              ...
                              title={t("LP token {asset}", { asset: lpAsset })}
                            >
                              {t("Pool")}
                            </a>
                          ) : (
                          ...
                          <span
                            ...
                            title={
                              poolLock.fullyLocked
                                ? t("Every LP token is burned — this liquidity can never be withdrawn.")
                                : t("{pct}% of the LP is burned. The rest can be withdrawn by whoever holds it.", { pct: poolLock.lockedPercent })
                            }
                          >
                            {t("{pct}% locked", { pct: poolLock.lockedPercent })}
                          </span>
                        </>
                      ) : (
                        <>
                    ...
                    <span className="relative z-10 ml-auto shrink-0 whitespace-nowrap text-gray-900 dark:text-gray-100">
                      {soldOut ? (
                        <span className="text-gray-400 dark:text-gray-500">0 · {t("sold out")}</span>
                      ) : (
                        <>
                          {quantityText}{" "}
                          <span className="text-gray-400 dark:text-gray-500">
      ...
      {tab === "orders" &&
        (!orders ? (
          <p className="p-6 text-center text-sm text-gray-400 dark:text-gray-500">{t("Loading order book…")}</p>
        ) : book.length === 0 ? (
          <p className="p-6 text-center text-sm text-gray-500 dark:text-gray-400">
            {t("No open orders on this pair — every trade here is going through the pool.")}
          </p>
        ) : (
          <>
            <div className="overflow-x-auto">
              ...
              <table className={PAIR_TABLE}>
                <thead>
                  <tr className="border-b border-gray-100 dark:border-gray-800 text-left text-[10px] font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">
                    ...
                    <th className="px-3 py-2 text-right">XCP</th>
                    <th className="px-3 py-2">{t("Address")}</th>
                    <th className="px-4 py-2 text-right">{t("Filled / expires")}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                  {ordered.slice(from, from + PER_PAGE).map((row, i) => {
                    ...
                    return (
                      ...
                      <tr className={mine ? "bg-purple-50/60 dark:bg-purple-950/40" : pool ? "bg-blue-50/40 dark:bg-blue-950/40" : undefined}>
                        ...
                        <td className="px-3 py-2">
                          {pool ? (
                            <span className="whitespace-nowrap text-xs text-gray-500 dark:text-gray-400">
                              {t("constant-product pool · 0.5% fee")}
                            </span>
                          ) : (
                          <span className="flex items-center gap-1.5 whitespace-nowrap">
                            <LaunchpadAddressHoverCard
                        ...
                        <td className="whitespace-nowrap px-4 py-2 text-right text-xs text-gray-500 dark:text-gray-400">
                          {pool ? (
                            <span title={t("A pool never runs out, it only gets worse. This is what it can absorb before its price moves 1%.")}>
                              {t("within 1%")}
                            </span>
                          ) : mine ? (
                            <button
