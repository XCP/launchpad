...
export function ScheduledPulse({
  ...
  useEffect(() => {
    ...
    document.title = open
      ? t("LIVE · {asset}", { asset })
      : remaining === 1
        ? t("{n} block · {asset}", { n: remaining, asset })
        : t("{n} blocks · {asset}", { n: remaining, asset });
    // Without this a soft navigation carries the countdown to the next page.
    return () => {
      document.title = previous;
    };
  }, [remaining, open, asset, t]);
    ...
    <div className="mt-8">
      <div className="py-2 text-center">
        ...
        <div className="mt-2.5 text-sm text-gray-500 dark:text-gray-400 tabular-nums">
          {open
            ? t("minting is live — refresh the page")
            : remaining <= 12
              ? t("{eta} until minting opens", { eta: blocksEta(remaining, t) })
              : t("until minting opens")}
        </div>
      </div>
        ...
        <div className="flex flex-row-reverse justify-start gap-2 overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_3rem)]">
          {upcoming.map((h, i) => (
            <BlockTile
              ...
              label={h === startBlock ? t("opens") : pendingEta(i + 1)}
