...
const PAGE_METADATA: Metadata = {
  title: msg("XCP.FUN - Bitcoin Memecoins on Counterparty"),
};
...
async function FirstLaunchHero() {
  const t = await getT();
  return (
    <div className="holo-border rounded-xl p-8 text-center">
      <h1 className="text-2xl font-bold">{t("Fairmint pools are live.")}</h1>
      <p className="mx-auto mt-3 max-w-xl text-gray-600 dark:text-gray-400">
        {t(
          "The first XCP-69 launch in history hasn't happened yet. All-or-nothing mints, at least {n} participants required, every raised XCP locked into the pool forever — enforced by consensus, not by this website.",
