...
function phaseWord(t: T, phase: SearchRow["phase"]): string {
  switch (phase) {
    case "graduated":
      return t("graduated");
    case "minting":
    ...
    >
      <D.Trigger asChild>
        ...
        >
          ...
          <span className="min-w-0 flex-1 truncate">{t("Search launches")}</span>
      ...
      <D.Portal>
        ...
        <D.Content className="modal-pop fixed left-1/2 top-[8vh] z-50 flex max-h-[80vh] w-[min(94vw,40rem)] -translate-x-1/2 flex-col overflow-hidden rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-2xl">
          <D.Title className="sr-only">{t("Search launches")}</D.Title>
          <D.Description className="sr-only">
            {t("Find a launch by asset name, and sort or filter the results.")}
          </D.Description>
          ...
          <div className="flex items-center gap-3 border-b border-gray-100 dark:border-gray-800 px-4 py-3.5">
            ...
            <input
              ...
              aria-label={t("Search asset name")}
              ...
              <li className="px-3 py-8 text-center text-sm text-gray-500 dark:text-gray-400">
                ...
                {rows === null
                  ...
                  : query && hiddenRefunded > 0
                    ? hiddenRefunded === 1
                      ? t("Nothing open matches “{query}” — {n} refunded launch hidden. Type a full name to open one.", { query, n: hiddenRefunded })
                      : t("Nothing open matches “{query}” — {n} refunded launches hidden. Type a full name to open one.", { query, n: hiddenRefunded })
                    : query
                      ? t("Nothing matches “{query}”.", { query })
                      : t("No launches in this phase yet.")}
              </li>
            ) : (
              results.map((r) => (
                <li key={r.asset}>
