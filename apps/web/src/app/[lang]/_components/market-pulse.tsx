...
/** The mark in the visitor's currency and number conventions. Bitcoin drops
 *  the minor units — at five figures they are noise — and XCP keeps whatever
 *  the currency has, which is two for most and none for yen. Past a million,
 *  as bitcoin is in yen or rupees, the chip cannot hold every digit, so it
 *  compacts to "¥12.44M" the way the cards' market caps do — and on a
 *  Japanese page that is "￥1247万", the way Japanese sites write it. */
...
function TickerButton({
  ...
  return (
    <button
      ...
      aria-label={isBtc ? t("Open Bitcoin price") : t("Open XCP price")}
    ...
    >
      ...
      <span className="flex min-w-0 flex-1 items-baseline gap-1.5">
        <span className="min-w-0 truncate text-sm font-bold tabular-nums text-gray-900 dark:text-gray-100">
          {price(market, value, code, rate, locale)}
        </span>
        {change30d !== null && (
          <span
            title={t("30-day change")}
      ...
      <D.Portal>
        ...
        <D.Content className="modal-pop fixed left-1/2 top-1/2 z-50 max-h-[calc(100dvh-2rem)] w-[calc(100vw-2rem)] max-w-2xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-3xl border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-950 p-3 shadow-2xl focus:outline-none sm:p-4">
          <div className="mb-3 flex items-center justify-between px-1">
            ...
            <div className="flex items-center gap-1">
              ...
              <button
                type="button"
                onClick={retry}
                aria-label={t("Refresh price chart")}
            ...
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400">{isBtc ? t("Bitcoin") : t("Counterparty")} ({code})</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-2xl font-bold tabular-nums text-gray-900 dark:text-gray-100 sm:text-3xl">
                  {price(market, spot, code, rate, locale)}
                </p>
                {change !== null && (
          ...
          <div className="mt-3 rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-3 shadow-sm sm:p-4">
            {points === null ? (
              <div className="flex h-64 items-center justify-center text-sm text-gray-400">{t("Loading price history…")}</div>
            ) : failed ? (
              <div className="flex h-64 flex-col items-center justify-center gap-3 text-center">
                <p className="text-sm text-gray-500 dark:text-gray-400">{t("Price history is temporarily unavailable.")}</p>
