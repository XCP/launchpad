  ...
  return (
    <article className="mx-auto max-w-6xl space-y-7 sm:space-y-8">
      <header className="max-w-3xl">
        <p className="text-xs font-semibold uppercase tracking-wide text-purple-600">XCP-69</p>
      ...
      <details className="group overflow-hidden rounded-xl border border-gray-200 bg-white">
        <summary className="flex cursor-pointer list-none items-center justify-between gap-4 p-4 marker:hidden sm:px-5">
          ...
          <span className="text-lg text-gray-400 transition group-open:rotate-45" aria-hidden="true">+</span>
        </summary>
        <div className="border-t border-gray-100 px-4 py-4 text-xs leading-relaxed text-gray-500 sm:px-5">
          <p>
      ...
      <footer className="flex flex-col gap-3 border-t border-gray-200 pt-5 text-xs text-gray-400 sm:flex-row sm:items-center sm:justify-between">
        <p>Observed address behavior, not identity or intent. Not investment advice.</p>
        <div className="flex gap-3">
          <Link href="/faq" className="text-purple-600 hover:underline">How XCP-69 works</Link>
          <a href="https://t.me/xcpfun" target="_blank" rel="noreferrer" className="text-purple-600 hover:underline">Corrections</a>
        </div>
      </footer>
    </article>
  );
